...
void pulse_gpio() {
    ...
    // Wait for echo rising edge (GPIO goes HIGH) with timeout
    ...
    t1 = read_timer_raw_macro2();  // Echo detected
    float dt_us = (double)(t1 - t0);
    float distance_cm = ((dt_us * SOUND_SPEED_CM_PER_US) / 2.0) - THRESHOLD;
...
float adc_read_dist() {
    const double SOUND_SPEED_CM_PER_US1 = 0.0344;
    adc_init();
    adc_gpio_init(26); 
    adc_select_input(0);
    ...
    for (int i = 0; i < max_samples; i++) {
        raw = read_stable_adc(10);
        if (raw < min_v) {
            t1 = read_timer_raw_macro2();
            float dt = (t1 - t0 - 100);
            float distance = ((dt * SOUND_SPEED_CM_PER_US1)/2) - THRESHOLD;
            printf("%.0f\n", distance);
...
int main() {
    ...
    while (1) {
        bool adc_mode = gpio_get(SET_ADC);
        bool pulse_mode = gpio_get(SET_PULSE);

        if (adc_mode == 0 && pulse_mode == 1) {
            pulse_gpio();
            sleep_ms(1);
        } else if (adc_mode == 1 && pulse_mode == 0) {
            adc_read_dist();
            sleep_ms(5);
        } else {
            oled_clear();
